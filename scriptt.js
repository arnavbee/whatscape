document.addEventListener("DOMContentLoaded", function () {
  ...
  const simulationDuration = 1000; // Duration for attracting nodes to the center (in milliseconds)
  ...
  const simulation = d3
    ...
    .force("center", d3.forceCenter(center.x, center.y)) // Center force to attract nodes to the center
    .alphaDecay(0); // Disable alpha decay for the initial attraction
  ...
  // Attract nodes to the center for a certain duration
  setTimeout(() => {
    simulation.alpha(1).alphaTarget(0).alphaDecay(0.0228).restart(); // Enable alpha decay to allow nodes to move
    node.call(
      d3
        .drag()
